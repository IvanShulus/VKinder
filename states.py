...
from messages import send_age, send_city, search_settings, change_city, only_digit, age_limit, searching
from utils import date_to_age, reversed_sex_table, sex_table, relation_table
...
async def age_handler(message: Message):
    ...
    if not text.isdigit():
        await message.answer(only_digit)
    else:
        age = int(text)
        if 1 <= age <= 100:
            ctx.set('age', age)
...
async def city_handler(message: Message):
    ...
    if cities.items:
        ...
        await message.answer(change_city)
...
async def check_user_data(message: Message, user_id):
    ...
    await add_new_user_in_db(user_id, age, sex_id, relation_id, city)
    ctx.delete('age')
    ctx.delete('city')
    await message.answer(search_settings.format(city, age, sex_text, relation_text))
    return True
