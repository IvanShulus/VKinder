...
async def search_users(user):
    ...
    for user_in_issuance in found_users:
        if user_in_issuance.is_closed:
            ...
            continue
        elif await is_viewed(user.user_id, user_in_issuance.id):
            continue
...
async def search(message):
    user = await get_user_from_db(message.from_id)
...
async def send_user(user, message, change_message=True, liked=False, last=False):
    pagination_keyboard = get_pagination_keyboard(user.id, last=last, liked=liked)
    ...
    if isinstance(message, Message):
        await message.answer(text=text, attachment=photos, keyboard=pagination_keyboard)
        return
    ...
    if change_message:
        await message.edit_message(message=text, attachment=photos, keyboard=pagination_keyboard)
    else:
        await message.send_message(message=text, attachment=photos, keyboard=pagination_keyboard)
...
async def next_user(event: MessageEvent):
    this_user_id = event.object.payload.get('user_id')
    liked_users = await api.users.get(this_user_id, fields=['sex', 'city', 'bdate', 'relation'])
    liked_user = liked_users[0]
    await send_user(liked_user, message=event, liked=True)

    user = await get_user_from_db(event.user_id)
    found_users = await search_users(user)
    if not found_users:
    ...
    await add_view_in_db(event.user_id, first_user_found.id)
    if len(found_users) < 2:
        await send_user(first_user_found, message=event, change_message=False, last=True)
        ctx.set('users', [])
        return
...
async def like_user(event: MessageEvent):
    ...
    liked_user = liked_users[0]
    await send_user(liked_user, message=event, liked=True)
    await add_like_in_db(event.user_id, liked_user_id)
